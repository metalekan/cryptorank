...
const SaveBtn = ({ data }) => {
  ...
  return (
    <button onClick={(event) => handleClick(event)} className="outline-0 bg-none cursor-pointer">
      <svg
...
const Saved = () => {
  ...
  return (
    <section className="w-full md:w-[80%] h-full flex flex-col my-16 mfb-24 relative">
      <div className="flex flex-col mt-9 border-2 border-gray-100 rounded-md overflow-auto min-h-[60vh] md:min-h-[0vh] stick left-0d">
        {
          savedData ? 
          <table className="w-full table-auto overflow-scroll">
            <thead className="capitalize text-base text-gray-100 font-medium border-b border-gray-100">
              <tr className="sticky top-0">
                <th className="py-1 bg-gray-300 rounded sticky left-0">
                  assets
                </th>
            ...
            <tbody className="">
              {savedData.map((data, index) => {
                return (
                  <tr
                    key={index}
                    className="text-center text-sm md:text-base border-b border-gray-100 last:border-b-0"
                  >
                    <td className="py-4 flex items-center uppercase sticky left-0 bg-gray-300 md:bg-[none] rounded bg-opacity-100 w-[150px]">
                      <SaveBtn data={data} />
                      <Link to={`/saved/${data.id}`}>
                        <div className="flex items-center">
                          ...
                          <div className="flex flex-col text-start text-sm">
                            <span className="capitalize text-clip overflow-hidden">
                              {data.name}
                            </span>
          ...
          <h1 className="min-h-[60vh] text-lg text-cyan flex items-center justify-center animate-pulse">
            There is no saved data!
          </h1>
        }
        <button className='w-[2rem] ml-4 hover:scale-110 transition-all transition-ease absolute right-0 -top-10' onClick={resetSavedData} >
