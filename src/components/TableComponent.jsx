                    ...
                    cryptoData ? <table className='w-full table-auto overflow-scroll'>
                        <thead className='capitalize text-base text-gray-100 font-medium border-b border-gray-100'>
                            <tr className='sticky top-0'>
                                <th className='py-1 bg-gray-300 rounded sticky left-0'>assets</th>
                                <th className='py-1'>price</th>
                                <th className='py-1'>market cap</th>
                                <th className='py-1'>volume (24h)</th>
                                <th className='py-1'>high 24H</th>
                                <th className='py-1'>1H</th>
                                <th className='py-1'>2H</th>
                        ...
                        <tbody className=''>
                            {
                                cryptoData.map((data, index) => {
                                    return (
                                        <tr key={index} className='text-center text-sm md:text-base border-b border-gray-100 last:border-b-0'>
                                            <td className='py-4 flex items-end uppercase sticky left-0 bg-gray-300 shadow-2xl md:shadow-none md:bg-[none] rounded bg-opacity-100 w-[150px] md:w-0'>
                                                <button className='outline-0 bg-none cursor-pointer'>
                                                ...
                                                <Link to={`/${data.id}`}>
                                                    <div className="flex items-center">
                                                        <img className='w-[2rem] h-[2rem] mx-1.5' src={data.image} alt={data.name} />
                                                        <div className="flex flex-col text-start text-sm">
                                                            <span className='capitalize text-clip overflow-hidden'>{data.name}</span>
                                                            <span className='text-gray-100'>{data.symbol}</span>
                                                        </div>
                                            ...
                                            <td className='py-4'>{new Intl.NumberFormat('en-US', { style: 'currency', currency: currency, }).format(data.current_price)}</td>

                                            <td className='py-4'>{new Intl.NumberFormat('en-US', { style: 'currency', currency: currency, notation: 'compact' }).format(data.market_cap)}</td>
                                            
                                            <td className='py-4'>{new Intl.NumberFormat('en-US', { style: 'currency', currency: currency, }).format(data.total_volume)}</td>

                                            <td className='py-4'>{new Intl.NumberFormat('en-Us', { style: 'currency', currency: currency }).format(data.high_24h)}</td>

                                            <td className={
                                                data.market_cap_change_percentage_1h < 0 ? "text-green py-4" : "text-red py-4"
                                            }>{Number(data.price_change_percentage_1h_in_currency).toFixed(5)}</td>
                                            <td className={
                                                data.price_change_percentage_24h_in_currency < 0 ? "text-green py-4" : "text-red py-4"
                                            }>{Number(data.price_change_percentage_24h_in_currency).toFixed(3)}</td>
                                            <td className={
                                                data.price_change_percentage_7d_in_currency < 0 ? "text-green py-4" : "text-red py-4"
                                            }>{Number(data.price_change_percentage_7d_in_currency).toFixed(3)}</td>
                                        </tr>
                                    )
                                })
                            }
